...
| GET | /api/projects/:id/wiki-index | project `viewer` + per-note read filter | 권한이 있는 프로젝트 노트의 live wiki index. 응답: `{ projectId, generatedAt, latestPageUpdatedAt, totals: { pages, wikiLinks, orphanPages }, links: [{ sourceNoteId, sourceTitle, targetNoteId, targetTitle }], unresolvedLinks: [{ sourceNoteId, sourceTitle, targetTitle, reason }], recentLogs: [{ noteId, noteTitle, agent, action, reason, createdAt }], pages: [{ id, title, type, sourceType, summary, updatedAt, inboundLinks, outboundLinks }] }` | - |
