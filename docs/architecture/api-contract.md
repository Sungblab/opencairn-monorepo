...
| GET | /api/internal/notes/:id/draft-state | `X-Internal-Secret` | Worker가 reviewable `note.update` 초안을 만들 때 사용하는 현재 note draft state. Yjs 문서가 있으면 Yjs state를 Plate value로 변환해 반환하고, 없으면 notes.content/contentText fallback에서 Yjs 문서를 backfill한 뒤 반환한다. 이후 `note.update` action 생성은 같은 `page:{noteId}` Yjs state vector를 기준으로 preview/apply한다. | `{ id, projectId, workspaceId, title, content, contentText, sourceType, sourceUrl, type, hasYjsDocument, yjsStateVectorBase64 }` |
