...
- Rerank: 기본은 deterministic heuristic rerank이며, 배포자가 `CHAT_RAG_RERANKER=provider`를 설정하면 active API chat provider가 evidence id 순서를 JSON으로 재정렬한다. provider rerank 실패 시 heuristic 결과를 그대로 사용한다.
...
- Chunk embedding은 raw chunk만이 아니라 page title과 heading path를 포함한 `context_text + content_text`로 생성한다. Prompt citation에는 raw chunk snippet을 유지하고, graph expansion 결과는 concept relation path를 metadata로 보존한다.
