...
> **2026-05-03 claim audit 반영**: 이 문서는 운영 전략과 구현 예정 작업을 함께 담고 있다. 현재 레포에는 `scripts/backup.sh`, `scripts/restore.sh`, `scripts/backup-verify.sh`가 아직 없으므로, public copy에서는 자동 백업/복구 스크립트가 제공된다고 말하지 않는다.
...
2. **Production은 WAL-G + R2 PITR**, 셀프호스트는 **pg_dump 기반 스크립트가 계획됨**
