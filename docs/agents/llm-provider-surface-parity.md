...
| --- | --- | --- | --- |
| Gemini | Native `google.genai` provider for chat, embeddings, tool calls, OCR, text-to-image generation, batch embedding plumbing, context-cache plumbing, Deep Research Interactions, and optional `GEMINI_SERVICE_TIER` inference routing. | Native API chat provider with Gemini streaming, embeddings, and optional `GEMINI_CHAT_SERVICE_TIER` / `GEMINI_SERVICE_TIER` inference routing. | Chat, Deep Research, and document-generation `imageEngine:"model"` flows consume API/worker outputs. Deep Research is Gemini-only. Service tiers are deployment-level knobs, not per-user product routing. |
