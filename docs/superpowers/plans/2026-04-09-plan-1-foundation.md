...
> **✅ 상태: 완료** (2026-04-20, HEAD `50eaf3b`) — Task 1 → Task 9까지 구현·리뷰·E2E 검증 통과. Docker postgres/redis/minio + Hono API + Next.js 16 web + Better Auth + workspace/project/folder/tag/note CRUD + 권한 3계층 (workspace/project/page) + 트랜잭션·XSS·UUID guard. 브라우저 플로우 signup → 워크스페이스 생성 → 노트 CRUD → soft-delete 전체 검증 완료. 권한 테스트 15/15 pass. 후속: Task B1 (백업 스크립트)은 배포 준비 단계에서 별도 진행. **다음 plan**: Plan 13 (multi-LLM provider).
>
> **커밋 범위**: `77d3919` → `50eaf3b` (총 19개 커밋). `git log 77d3919^..50eaf3b` 참고.
...
- [x] **Step 3: Verify PostgreSQL connection**
...
- [x] **Step 3: Create packages/db/drizzle.config.ts**
...
- [x] **Step 4: Create packages/db/src/schema/custom-types.ts**
...
- [x] **Step 5: Create packages/db/src/schema/enums.ts**
...
- [x] **Step 6: Create packages/db/src/schema/users.ts**
...
- [x] **Step 7: Create packages/db/src/schema/auth.ts**
...
- [x] **Step 8a: Create packages/db/src/schema/workspaces.ts**
...
- [x] **Step 8b: Create packages/db/src/schema/workspace-members.ts**
...
- [x] **Step 8c: Create packages/db/src/schema/workspace-invites.ts**
...
- [x] **Step 8d: Create packages/db/src/schema/projects.ts (workspace_id FK)**
...
- [x] **Step 8e: Create packages/db/src/schema/project-permissions.ts**
...
- [x] **Step 8f: Create packages/db/src/schema/page-permissions.ts**
...
- [x] **Step 9: Create packages/db/src/schema/folders.ts**
...
- [x] **Step 10: Create packages/db/src/schema/tags.ts**
...
- [x] **Step 11: Create packages/db/src/schema/notes.ts**
...
- [x] **Step 12: Create packages/db/src/schema/concepts.ts**
...
- [x] **Step 13: Create packages/db/src/schema/wiki-logs.ts**
...
- [x] **Step 14: Create packages/db/src/schema/learning.ts**
...
- [x] **Step 15: Create packages/db/src/schema/jobs.ts**
...
- [x] **Step 16: Create packages/db/src/schema/conversations.ts**
...
- [x] **Step 17: Create packages/db/src/client.ts**
...
- [x] **Step 18: Create packages/db/src/index.ts**
...
- [x] **Step 19: Install deps, generate migration, run migration**
...
- [x] **Step 3: Create packages/shared/src/api-types.ts**
...
- [x] **Step 4: Create packages/shared/src/constants.ts**
...
- [x] **Step 3: Create apps/api/src/lib/db.ts**

```typescript
export { db } from "@opencairn/db";
```

- [x] **Step 4: Create apps/api/src/lib/auth.ts**
...
- [x] **Step 5: Create apps/api/src/middleware/error.ts**
...
- [x] **Step 6: Create apps/api/src/middleware/auth.ts**
...
- [x] **Step 7: Create apps/api/src/routes/health.ts**
...
- [x] **Step 8: Create apps/api/src/routes/auth.ts**
...
- [x] **Step 9: Create apps/api/src/app.ts**
...
- [x] **Step 10: Create apps/api/src/index.ts**
...
- [x] **Step 11: Install deps, start server, verify health endpoint**
...
- [x] **Step 1: permissions.ts 작성 (핵심 헬퍼)**
...
- [x] **Step 3: workspaces.ts CRUD + 멤버 관리**
...
- [x] **Step 4: invites.ts — 초대 생성·수락·거절**
...
- [x] **Step 5: `apps/api/src/lib/email.ts` — Resend 래퍼**
- [x] **Step 6: `apps/api/src/app.ts`에서 라우트 마운트**
...
- [x] **Step 6.5: permissions.ts 유닛 테스트**
...
- [x] **Step 1: Create apps/api/src/routes/projects.ts**
...
- [x] **Step 2: Add zod-validator dep**

```bash
cd apps/api && pnpm add @hono/zod-validator && cd ../..
```

- [x] **Step 3: Mount project routes in apps/api/src/app.ts**
...
- [x] **Step 4: Verify with curl** — workspace 생성 후 project 생성까지 flow 확인
...
- [x] **Step 2: Create apps/web/tsconfig.json**
...
- [x] **Step 3: Create apps/web/next.config.ts**
...
- [x] **Step 4: Create apps/web/postcss.config.mjs**
...
- [x] **Step 5: Create apps/web/src/app/globals.css**

```css
@import "tailwindcss";
```

- [x] **Step 6: Create apps/web/src/app/layout.tsx**
...
- [x] **Step 7: Create apps/web/src/app/page.tsx**
...
- [x] **Step 8: Create apps/web/src/app/(app)/layout.tsx**
...
- [x] **Step 9: Create apps/web/src/app/(app)/dashboard/page.tsx**
...
- [x] **Step 10: Install deps, start dev server, verify**
...
- [x] **Step 1: Create API proxy route handler**
...
- [x] **Step 2: Create API client (TanStack Query에서 사용)**
...
- [x] **Step 3: Add INTERNAL_API_URL to .env.example**
...
- [x] **Step 2: Verify health endpoint**
