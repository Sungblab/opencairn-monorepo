...
| `source` | 브라우저 네이티브 PDF 뷰어 + 앱 toolbar chrome | R2 URL → PDF |
...
**Tech Stack:** Plate v49 (Yjs 플러그인 + comments 플러그인), shadcn/ui, KaTeX, @platejs/math (MathKit), **Yjs**, **Hocuspocus 서버 + Provider + Awareness**, TanStack Query, Tailwind CSS 4, Hono 4, Zod, React 19, Next.js 16, Resend (이메일 알림), **react-json-view** (JSON 뷰어), **브라우저 네이티브 PDF 렌더러** (PDF), **isomorphic-dompurify** (artifact 정화).
...
apps/web/
  ...
  components/
    source-viewer/
      ...
      PdfViewer.tsx                 -- browser-native PDF embed (내장 검색/줌/페이지 탐색)
      HtmlViewer.tsx                -- iframe + sandbox 속성 (HTML 파일 안전 렌더링)
```

> **Source Viewer 뷰어 라이브러리:**
> - PDF: 브라우저 네이티브 PDF 뷰어 (내장 툴바/줌/페이지 네비/검색) + 앱 toolbar chrome
