...
| 기존 도구 | 한계 | OpenCairn |
|-----------|------|-----------|
| Notion | 수동 정리, AI는 보조, SaaS 전용 (규제 산업 진입 불가) | AI가 위키 자동 구축 + 셀프호스팅 + Ollama 완전 로컬 |
| NotebookLM | 구글 종속, 셀프호스팅 불가, 협업 없음 | 오픈소스, 셀프호스팅, 팀 협업 + 12개 에이전트 |
| Obsidian | 로컬 마크다운, AI 자동 없음, 협업 약함 | 네이티브 AI, 자동 위키 컴파일, Yjs 실시간 협업 |
| ChatGPT/Claude | 세션 기반, 지식 축적 불가 | 대화 결과가 위키로 누적, 지식이 영속됨 |
| MiroFish/시뮬레이션 도구 | 외부 세계 예측 전용, 개인 지식과 무관 | 내 지식베이스로 시나리오 시뮬레이션 + 지식 인터뷰 |

**독보적 차별화 기능 (타 서비스 없음):**
- **팀 지식 위에서 도는 AI 에이전트**: Notion AI는 단일 페이지 반응, OpenCairn은 조직 전체 위키를 Compiler/Synthesis/Curator가 24시간 유지.
- **셀프호스팅 + 듀얼 라이선스(AGPLv3 + 상용) + Ollama**: 규제 산업(금융·의료·공공·대학·법무)이 Notion 못 쓰는 이유를 해결. 내부 OSS 정책상 AGPL이 어려운 조직에는 상용 라이선스 옵션. 데이터 외부 송신 0.
...
> **Notion은 네가 쓴 것만 담는다. OpenCairn은 네가 읽은 것까지 이해한다.**
>
> 자료를 던져두면 AI가 위키로 엮고, 네가 묻기 전에 연결을 발견하고, 너의 이해가 얼마나 깊은지 스스로 측정한다. 팀과 함께 쓰면 회사의 집단 지능이 된다. 셀프호스팅으로 데이터는 너의 것이다.
...
v0.1에서 모두를 노리는 실수를 피하기 위해 단계별 primary를 명시한다. 기능은 전부 갖추되 **마케팅 초점과 온보딩 경로**가 단계별로 다름.

| 단계 | Primary | 확장 대상 | 도입 경로 |
|------|---------|---------|----------|
| **v0.1** | 대학원생 / 리서처 (개인) | 얼리어답터 개발자 | 본인 셀프호스트, Docker 한 방 |
| **v0.2** | 연구실 / 스타트업 (3~15명 팀) | 오픈소스 커뮤니티 | "내 연구실 Notion 대체" grass-roots |
| **v0.3** | 규제 산업 (금융·의료·공공·대학) | 컴플라이언스 팀 | 상용 라이선스 + Enterprise 지원 |

### P1: 대학원생 / 리서처 (Primary — v0.1 타겟)
...
- Notion은 AI 부재, NotebookLM은 프로젝트 간 연결 불가, Obsidian은 AI 플러그인이 조각조각
**Job to be Done**:
"논문을 올리면 자동으로 정리하고, 개념 간 관계가 보이고 시험/논문 작성까지 도와주는 도구"

**Success Criteria**:
- 논문 PDF 업로드 후 5분 내 위키 페이지 자동 생성
- 지식 그래프에서 논문 간 관계 시각화 확인
- 플래시카드로 핵심 개념 복습
- 논문 작성 시 위키에서 `\cite{kg:vaswani2017}` 자동 resolve

**왜 v0.1 Primary인가:**
- 가격 민감도 높아 셀프호스팅 선호
- Obsidian으로 이미 "개인 지식 관리" 도구에 익숙
- 학계 네트워크 = grass-roots 바이럴 경로
- Deep Research + KG가 바로 가치

---

### P2: 연구실 / 소규모 팀 (v0.2 확장)

**이름**: 박교수 연구실 (교수 1 + 석박사 8명)
**상황**: 랩 미팅 자료, 논문, 실험 노트, 코드가 각자 로컬 또는 카카오톡에 흩어져 있음
**Pain Point**:
- 선배가 졸업하면 그가 축적한 지식이 사라짐 (knowledge churn)
- 신입이 6개월간 연구실 맥락 파악에 비효율
- Notion 도입하고 싶지만 민감한 데이터(연구 가설, 미공개 결과)라 클라우드 불가
**Job to be Done**:
"랩 전체의 집단 지능 플랫폼. 실험실 밖으로 데이터 안 나가고, AI가 랩 지식을 학습해 질문에 답"

**Success Criteria**:
- 랩실 서버에 docker-compose 한 번으로 배포
- 교수·학생 각자 계정 + 역할 (교수 admin, 학생 member)
- 랩 미팅 녹음 업로드 → 자동 요약 + 액션 아이템 추출
- 신입이 "이 실험 왜 하는 거예요?"를 AI에게 물어 답받음

---

### P3: 규제 산업 엔터프라이즈 (v0.3 타겟)

**이름**: 한국 시중은행 IT기획팀 (50명 규모)
**상황**: 사내 정책·규정·기술 문서가 여러 시스템에 파편화. Notion 못 씀 (금융 규제).
**Pain Point**:
- 은행법·전자금융거래법에 따라 고객 정보·내부 전략은 외부 SaaS 금지
- 기존 사내 Wiki (Confluence)는 검색 품질 낮고 AI 없음
- 신입 교육 온보딩 3개월 걸림
**Job to be Done**:
"온프레미스 셀프호스트 + Ollama 완전 로컬 + SSO + 감사 로그 → 전사 지식 베이스"

**Success Criteria**:
- Ollama로 외부 LLM 호출 0 (보안팀 승인)
- SAML SSO로 기존 AD 계정 연동
- 전 활동 감사 로그 export (분기별 컴플라이언스 심사)
- 200명 동시 접속 처리 (Hocuspocus 부하)
- 상용 라이선스 계약 (듀얼 라이선스 트랙 — ADR-005, `LICENSE-COMMERCIAL.md`)
...
| ID | User Story | Priority | Acceptance Criteria |
|----|-----------|----------|---------------------|
| KG-01 | 사용자로서, 내 지식의 개념 관계를 그래프로 시각화한다 (Graph 뷰) | P0 | Cytoscape.js cose-bilkent/fcose, 줌/패닝/드래그, 자동 클러스터링 |
...
### 3.13 협업 (Workspace + Permissions + Real-Time, v0.1 포함)

v0.1부터 데이터 모델·실시간 편집·기본 협업 UX를 갖춘다. v0.2 이상은 UI 완성도·추가 기능. 상세 설계: [collaboration-model.md](../../architecture/collaboration-model.md).

| ID | User Story | Priority | Acceptance Criteria |
|----|-----------|----------|---------------------|
| WS-01 | 사용자로서, 여러 워크스페이스(개인·회사·연구실)를 분리해 운영한다 | P0 | Workspace switcher UI, 데이터 완전 격리 |
| WS-02 | Admin으로서, 멤버를 이메일로 초대한다 | P0 | Invite 이메일 발송, 토큰 링크로 수락, 7일 만료 |
| WS-03 | Admin으로서, 멤버 역할(admin/member/guest)을 변경한다 | P0 | UI에서 권한 변경, activity_events에 기록 |
| WS-04 | 편집자로서, 특정 프로젝트에 다른 멤버의 접근을 제한한다 | P0 | project_permissions override, viewer / editor 선택 |
| WS-05 | 편집자로서, 특정 페이지만 비공개로 설정한다 (Notion 방식) | P1 | page_permissions + inherit_parent=false |
| WS-06 | 사용자로서, 다른 멤버와 같은 페이지를 실시간 공동 편집한다 | P0 | Yjs + Hocuspocus, 커서·아바타 표시 |
| WS-07 | 사용자로서, 페이지를 보고 있는 다른 멤버가 누구인지 본다 | P1 | Presence avatars on page top |
| WS-08 | 사용자로서, 블록에 인라인 코멘트를 단다 | P0 | block anchor + thread + resolve |
| WS-09 | 사용자로서, 코멘트에서 @user / @page / @date 를 멘션한다 | P0 | Plate 플러그인 combobox, 저장 시 mention 관계 생성 |
| WS-10 | 사용자로서, 나를 언급한 코멘트가 오면 인앱·이메일로 알림받는다 | P0 | `notifications` 테이블 + SSE + Resend 이메일 |
| WS-11 | 사용자로서, 알림 선호도(즉시/일간/끔)를 타입별로 설정한다 | P1 | notification_preferences UI |
| WS-12 | 사용자로서, 워크스페이스에서 누가 무엇을 했는지 타임라인을 본다 | P1 | activity_events 피드 (user + agent 활동 통합) |
| WS-13 | 편집자로서, 페이지를 공개 링크로 공유한다 | P1 | public_share_links, viewer/commenter 선택, 암호·만료 옵션 |
| WS-14 | Guest로서, 초대받은 페이지만 접근한다 | P2 | 다른 리소스는 API 404, 멤버 리스트 숨김 |
| WS-15 | 사용자로서, 워크스페이스 소유권을 다른 멤버에게 이전한다 | P2 | owner 이전 플로우, 이메일 확인 |

### 3.14 자율 수집 (Hunter Agent — v0.2 이관)

> **범위 조정 (2026-04-14):** Hunter Agent는 v0.1 범위에서 제외됨. 유사 기능(웹 자료 추천)은 **Curator Agent + Gemini Search Grounding**이 담당 (§3.12 참조). Hunter는 브라우징 자동화 기반 대용량 수집이 필요한 v0.2 기능으로 이관. 본 절의 유저스토리는 v0.2 설계 기준으로만 유효.

| ID | User Story | Priority | Acceptance Criteria |
|----|-----------|----------|---------------------|
| HT-01 (v0.2) | 사용자로서, 에이전트에게 특정 주제 수집(브라우징)을 지시할 수 있다 | v0.2 | 자율 브라우징(Playwright) 기반 문서 다운로드 및 인제스트 |
| HT-02 (v0.2) | 사용자로서, 특정 웹사이트를 주기적으로 크롤링하도록 스케줄링할 수 있다 | v0.2 | 크론 스케줄링 기반 루틴 -> 자동 컴파일 및 위키 병합 |
...
| AUTH-02 | 사용자로서, Free/Pro/BYOK 플랜을 선택한다 | P0 | Toss Payments Checkout (한국 원화 구독) |
...
- 캔버스 (브라우저 Pyodide + iframe sandbox, ADR-006)

### Phase 4: Polish & Launch (1주)

- 랜딩페이지 + 블로그
- 빌링 (Toss Payments, 한국 원화)
- 나머지 에이전트 (Librarian, Connector, Temporal, Synthesis, Curator, Deep Research, Code, Visualization)
- 문서
- v0.1 공개
