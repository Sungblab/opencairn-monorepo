...
- **결제 경로 2가지를 모두 지원** — (a) BYOK: 사용자 Gemini 키로 직접 호출, (b) Managed PAYG: 서버 계정 키로 호출하고 사용자 PAYG 크레딧 차감. 둘 다 한쪽만 있으면 충분 (둘 다 없으면 `/settings/ai` 또는 `/settings/billing` 링크로 ungating). 경로 선택 로직은 §7 참고.
...
- **PAYG 실제 과금/정산 구현** — Plan 9b BLOCKED 상태. 본 spec은 PAYG 경로의 **아키텍처/인터페이스만 정의**하고, 실제 크레딧 차감·PG 연동·영수증은 Plan 9b 후속. 출시 시점에는 BYOK 경로만 live, PAYG 경로는 feature flag 뒤 no-op.
...
export const researchRuns = pgTable("research_runs", {
  ...
  billingPath: researchBillingPathEnum("billing_path").notNull(),   // byok | managed (§7)
...
## 7. Access & Billing Paths

Deep Research 호출을 위한 Gemini API 키를 공급하는 경로가 2가지다. **접근 제한은 없으며** (파워 유저든 솔로든), 어느 한쪽이 사용 가능하면 기능을 쓸 수 있다.

### 7.1 경로 A — BYOK (사용자 키)

- 사용자가 Settings에 등록한 Gemini API 키 (AES-256 암호화, Plan 13 기존 인프라)
- 호출 시 `GeminiProvider(api_key=decrypted_byok)` 인스턴스로 직접 호출
- **비용 청구는 Google → 사용자 Google Cloud 계정**. 우리는 중간 정산 관여하지 않음.
- **게이팅 없음** — 월 몇 번 · 하루 몇 번 · 모델 제한 등 선제 차단 안 함 (Feedback: 2026-04-22 `feedback_byok_cost_philosophy.md`)
- 실제 비용은 Google Cloud Console에서 확인 안내

### 7.2 경로 B — Managed PAYG (서버 키 + 사용자 크레딧)

- Pro 사용자가 PAYG 크레딧을 충전하면 사용 가능 (Plan 9b 후속)
- 서버 계정 Gemini API 키(`GEMINI_MANAGED_API_KEY` env)로 호출
- Temporal workflow가 run 시작 전 **holding reserve**: 모델별 상한 추정치(Max면 $7.00 × 마진 × KRW 환율 = ₩약 13,000)만큼 크레딧 잔액 선차감 예약 (DB `credit_reservations` 테이블, Plan 9b 도입)
- run 완료 후 추정 비용(§7.4)으로 정산, 차액 환원 또는 보정
- 잔액 부족 시 run 생성 거부 — "크레딧 충전이 필요합니다" 안내 + `/settings/billing` 링크
- **게이팅은 있지만 "사용자 보호" 목적 아님** — 우리가 선결제 받아야 하는 관리형 경로 고유 제약

### 7.3 경로 선택 로직

run 생성 시 사용자가 선택하되, 사용 가능한 경로만 노출:

```
사용자 상태                       | 선택 가능           | Default
──────────────────────────────────|─────────────────────|─────────
BYOK 키만 등록                     | BYOK                | BYOK
PAYG 크레딧만 있음                 | Managed             | Managed
둘 다 있음                         | BYOK / Managed 선택 | BYOK (사용자가 직접 지불하는 쪽 우선)
둘 다 없음                         | 없음                | (CTA로 막힘)
```

- 선택은 `research_runs.billingPath` enum 컬럼으로 기록 (`byok` | `managed`)
- UI 토글은 단순 라디오 버튼 2개 + "이번 달 잔여 크레딧 표시" (Managed 선택 시)

### 7.4 비용 추정 (양 경로 공통)

Google이 실제 비용을 API로 반환하지 않으므로 **추정치**로 기록한다.

- Pre-run: 모델 선택 시 "Deep Research: ~$1–3, Max: ~$3–7" 안내 (i18n 카피 고정)
- Post-run: `research_runs.totalCostUsdCents`에 추정치 기록, `research-meta` 블록에 "approx" 라벨 + 고지문
- 추정 공식:
  ```
  ...
  ```
- BYOK: 추정치는 **정보 표시만** (실제 청구는 Google Cloud Console에서)
- Managed: 추정치에 **마진율**(env `MANAGED_MARGIN`, 기본 1.3) 적용한 값이 사용자에게 청구되는 금액. 차액은 운영 마진. Plan 9b에서 정식화.

### 7.5 MVP 출시 범위 (Plan 9b 전)

- 경로 A (BYOK) **완전 구현 · live**
- 경로 B (Managed PAYG) **아키텍처/인터페이스 구현, feature flag 뒤 비활성화**
  - DB 컬럼 · 서비스 키 로딩 · 경로 분기 코드는 존재
  - `FEATURE_MANAGED_DEEP_RESEARCH=false` 기본 → UI 토글 미노출, run 생성 시 `billingPath="byok"` 강제
  - Plan 9b 완료 시 `credit_reservations` 연동 1줄 추가 + flag on → 즉시 live

**이것이 "BYOK only 아님"의 구현 방식**: 설계는 양쪽, 출시는 한쪽부터. 재작업 없음.

### 7.6 MVP 밖 → Spec B로 분리
...
- `FEATURE_MANAGED_DEEP_RESEARCH` env var (기본 `false`, Plan 9b 완료 시 `true`)
  - flag off: run 생성 시 BYOK만 가능 (UI 토글 미노출, `billingPath="byok"` 강제). 서버 계정 키 `GEMINI_MANAGED_API_KEY`는 로드만 하고 미사용.
  - flag on: 사용자가 PAYG 크레딧 보유 시 Managed 경로 선택 가능
- 접근 제약 (우선순위):
  1. `FEATURE_DEEP_RESEARCH` off → 404
  2. 사용자에게 BYOK 키도 없고 (`FEATURE_MANAGED_DEEP_RESEARCH` off 또는 크레딧 0) → 허브에서 CTA:
     - BYOK 미등록: "Gemini API 키를 먼저 설정해주세요" → `/settings/ai`
     - Managed 가능 상태에서 크레딧 부족: "크레딧 충전이 필요합니다" → `/settings/billing`
