...
TEMPLATES = {
    # 명시적으로 사용자에게 보이는 이벤트
    ("research", "tool_call", "hybrid_search"):
        lambda e: f"‘{e.args['query']}’ 관련 문서 훑는 중…",
    ("research", "tool_end",   "hybrid_search"):
        lambda e: f"{len(e.result)}건 찾음",
    ("research", "tool_call", "fetch_page"):
        lambda e: f"노트 열어보는 중: ‘{e.args['page_title']}’",
    ("compiler", "tool_call", "extract_concepts"):
        lambda e: "개념 추출 중…",
    ...
    # 오류 / 방향 전환
    ("*", "tool_end_error", "*"):
        lambda e: f"{e.tool_name} 실패 → 다른 방법 시도",
    ("*", "retry", "*"):
...
Plan 12의 `AgentEvent` enum에 **추가** (기존 9개 유지):

```python
class AgentEventType(str, Enum):
    # ... 기존 9개
    thought_summary    = "thought_summary"     # Gemini thinking 델타
    phase_transition   = "phase_transition"    # 에이전트 단계 전환
    status_line        = "status_line"         # Humanizer 생성 한 줄 (서버 생성)
    route_decision     = "route_decision"      # 모델 라우터 결정 (별도 spec)
```
...
1. **에이전트가 방향 전환할 때**: 재시도 정보를 Phase transition으로 뺄지, 단순 status line으로 둘지 — 사용자 관찰 데이터 필요
2. **모바일 UX**: status line이 접힌 채로 기본, 펼치면 최근 5개? 아니면 항상 1줄만?
3. **사용자 개입 지점**: status line에 "취소" 버튼을 상시 노출할지, 아니면 N초 이상 걸릴 때만? Cost 절약 vs 혼란
4. **로컬 스토리지**: 에이전트 실행 중 status line 히스토리를 저장할지 (디버깅 용) — privacy 관점 검토 필요
