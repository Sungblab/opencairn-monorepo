...
### Tier 1 — 배포 전 (구조적 강화) — 🟡 IN PROGRESS (branch `fix/plans-1-to-4-tier-1`)

| # | 상태 | 작업 |
|---|:-:|---|
| 1-1 | ⏳ | concepts/merge: `sql.raw` → `sql.join` + 진짜 `db.transaction` + workspaceId 강제. 다음 커밋. **[Plan 4 C-1 + 1 H-5]** |
| 1-2 | ⏳ | 세마포어 acquire/renewal `db.transaction` + `pg_advisory_xact_lock(hashtext(projectId))`. C-1 이후 커밋. **[Plan 4 C-2]** |
| 1-3 | 🟡 | `assertResourceWorkspace` 헬퍼 + 단위 테스트 커밋됨 (`f585231`). internal.ts 라우트별 `workspaceId` 적용 + worker 전파는 후속 세션 (B-후속). 우선순위: 쓰기 라우트 9개 (upsert/edges/notes-link/wiki-logs/notes-PATCH/refresh-tsv + semaphores A/R + merge). **[Plan 4 C-3 + H-1 + H-4 + M-3]** |
| 1-4 | ✅ | `yt_dlp.YoutubeDL.extract_info` 전 hostname 화이트리스트 재검증 + `_assert_youtube_url`. 16 새 테스트. (`2b4f839`) **[Plan 3 C-2]** |
| 1-5 | ✅ | `ingest_jobs` 테이블 (migration 0016) + `/ingest/status/:workflowId` owner check. 4 새 테스트 (404/200/403/401). (`7eb4849`) **[Plan 3 H-1]** |
| 1-6 | ✅ | `POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:?...}` + DATABASE_URL fallback 재구성 + .env.example 확장. (`0dcdf4f`) **[Plan 1 C-6]** |
| 1-7 | ✅ | `googleOAuthEnabled` gate: API `socialProviders` + `oneTap()`, web `oneTapClient` + GoogleButton + GoogleOneTap + 3 form divider 모두 env 조건부. (`26dc818`) **[Plan 1 C-3]** |

**Wave B 후속** (`fix/plans-1-to-4-tier-1` 또는 후속 PR):

1. internal.ts 쓰기 라우트에 `workspaceId` 파라미터 추가 — 우선순위 순:
   - `POST /concept-edges` — `sourceId.projectId === targetId.projectId` 검증 포함
   - `POST /concept-notes` — 동일 프로젝트 검증 포함
   - `POST /concepts/upsert`
   - `POST /wiki-logs`
   - `PATCH /internal/notes/:id`
   - `POST /notes/:id/refresh-tsv`
   - `POST /embedding-batches` / `PATCH /embedding-batches/:id`
   - `PATCH /import-jobs/:id`
   - `POST /internal/projects` — workspaceId 이미 있음, 추가 검증만
2. `apps/worker/src/worker/lib/api_client.py` `AgentApiClient` 메서드 9개 시그니처에 `workspace_id` 추가 (콤퍼일러/리브러리언/리서치/배치 embed 경로).
3. 워커 워크플로우 페이로드 → 에이전트 → activity → api_client 로 `workspace_id` 전파. 대부분 이미 payload 에 있음 (`deep_research_workflow.py:49`, `import_workflow.py:44`, `batch_embed_workflow.py:83`).
4. 테스트 업데이트: `test_compiler_agent.py`/`test_librarian_agent.py`/`test_batch_embed_activities.py` 의 mock 호출 시그니처.
