...
### Tier 2 — Migration 0014 (스키마 묶음)

| # | 작업 |
|---|---|
| 2-1 | `CREATE INDEX notes_embedding_hnsw ON notes USING hnsw (embedding vector_cosine_ops)` **[Plan 4 M-1]** |
| 2-2 | `CREATE INDEX concepts_embedding_hnsw ON concepts USING hnsw (embedding vector_cosine_ops)` **[Plan 4 M-1]** |
| 2-3 | `CREATE UNIQUE INDEX workspace_invites_ws_email_pending ON workspace_invites (workspace_id, email) WHERE accepted_at IS NULL` **[Plan 1 H-1]** |
| 2-4 | `CREATE INDEX workspace_members_invited_by_idx ON workspace_members (invited_by)` (+ `workspace_invites.invited_by` 도 동일) **[Plan 1 H-2]** |
| 2-5 | `ALTER TABLE workspaces ADD CONSTRAINT workspaces_slug_lower CHECK (slug = lower(slug))` **[Plan 1 H-6]** |
| 2-6 | `DROP INDEX workspaces_slug_idx` (unique constraint 자동 인덱스와 중복) **[Plan 1 H-7]** |
| 2-7 | `ALTER TABLE yjs_documents ADD COLUMN size_bytes integer` + CHECK (< 4MB 등). persistence.ts 에 cap 검사 추가 **[Plan 2B H-3]** |
