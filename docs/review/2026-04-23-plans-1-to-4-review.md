...
**프로덕션 주의**: 2-7 의 `CHECK (octet_length(state) <= 4 MB)` 은 기존 row 가 cap 을 초과하면 `ALTER TABLE` 단계에서 실패. 실제 배포 전 `SELECT max(octet_length(state)) FROM yjs_documents;` 로 pre-flight 검증 필요. dev 에서는 문제 없었음 (최대값 < 100 bytes).
