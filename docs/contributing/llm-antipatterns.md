...
- ❌ Ollama tool 호출을 text-only fallback으로 처리 → ✅ `OllamaProvider.generate_with_tools`는 공식 `/api/chat` `tools` / `message.tool_calls` / `role="tool"` 응답 루프를 사용해야 함. Tool을 요구했는데 content만 읽고 넘어가면 agent guard와 tool accounting이 깨짐.
...
## 15. Shell hooks, Plate editor, tab keyboard (App Shell Phase 3, 2026-04-24)

Added 2026-04-24. App Shell Phase 3-A(`apps/web/src/hooks/use-tab-*`, `apps/web/src/components/tab-shell/*`) 구현 중 발견.

- ❌ URL ↔ store reconciliation 훅(`useUrlTabSync`)을 ShellProviders에서도 마운트하고 그 아래 `TabBar` 하위 컴포넌트(`SortableTab`, `TabOverflowMenu`)에서도 `const { navigateToTab } = useUrlTabSync()`로 재호출 → ✅ 같은 훅을 여러 지점에서 마운트하면 **URL→tab 동기화 effect가 중복 실행**. 테스트에서 "탭 1개 추가 후 render했는데 결과가 탭 2개"가 되어서 `findTabByTarget`이 현재 URL의 kind/targetId를 찾지 못하자 `addTab`을 한 번 더 호출한 것. 순수 navigator(router+slug→url)만 필요하면 별도 훅으로 분리할 것. 우리 코드는 `useTabNavigate`로 추출 — ShellProviders에서만 full `useUrlTabSync`를 마운트하고, 그 외 모든 소비자는 navigator-only 훅을 쓴다.
- ❌ `<Plate>` 내부에 `<div onKeyDownCapture={() => promote()}>` 같이 **모든 keydown으로 preview 승격** → ✅ 화살표 키/Tab/Escape/⌘S/⌘A/Shift 단독 등 non-edit 키에도 fire됨. "사이드바에서 노트 프리뷰 열고 커서 둘러보기만 해도 sticky가 됨". 필터 조건: `e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey` 또는 `e.key ∈ {Backspace, Delete, Enter}`. 알려진 한계: 마우스 편집(우클릭 paste, slash-command 블록 삽입, drag-drop 이미지)은 키보드 event가 없어서 승격 안됨 — 완전 커버는 Plate `editor.onChange` 경로를 훅할 때.
- ❌ `useKeyboardShortcut("mod+ArrowLeft", handler)` / `window.addEventListener("keydown", ...)` 로 탭 switch 단축키를 건 뒤, 에디터 포커스 가드 없이 `e.preventDefault()` + 액션 실행 → ✅ 화살표는 **input / textarea / contenteditable의 cursor navigation과 충돌**. Cmd+← 는 줄 시작, Alt+← 는 word-back. 탭 단축키는 `document.activeElement`(또는 `e.target`)의 editable 여부를 가드해야 함. 구현: `isEditableTarget(e.target)` 체크 후 arrow만 bail, Cmd+W/Cmd+T/Cmd+1-9 같은 non-editor 체이니는 그대로 fire.
- ❌ Playwright 셀렉터를 `[data-testid^="tab-"]`로 걸어 탭 row들을 카운트 → ✅ 같은 prefix의 `tab-bar`, `tab-bar-new`, `tab-overflow-trigger` testid도 전부 매칭됨. 절대 count(`toHaveCount(0)` 등) 어싱션이 2-3개 오프. 컴포넌트 role을 활용해 `[role="tab"]`로 타깃하거나 tab-factory의 `t_` prefix를 활용해 `[data-testid^="tab-t_"]`로 좁힐 것. testid 네이밍에서 `-`는 boundary가 아니므로 prefix 매칭의 한계를 인지할 것.
