...
- **Call [`count_tokens`](https://ai.google.dev/api/rest/v1/models/countTokens) with the input
  of the request.**  
  This returns the total number of tokens in _the
  input only_. You can make this call before sending the input to the
...
- **Use the `usage_metadata` attribute on the `response` object after
  ...
  tokens in _both the input and the output_ : `total_token_count`.  
  It
...
the text in _the input only_ (`total_tokens`). You can make this call before
...
- The total number of tokens in _both the input and the output_
  (`total_token_count`)
...
token count of the text and the image in _the input only_ (`total_tokens`). You
...
combined token count of the text and the video/audio file in _the input only_
