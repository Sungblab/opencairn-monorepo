...
async def test_fetch_explicit_only_no_auto_search():
    ...
    async def fake_fetch(_workspace_id: str, source_id: str):
        return {
            "id": source_id,
            "title": f"T-{source_id}",
            "body": "x" * 100,
    ...
    with (
        patch("worker.activities.synthesis_export.fetch.set_status", new=AsyncMock()),
        patch(
            "worker.activities.synthesis_export.fetch._fetch_s3_object",
            new=AsyncMock(side_effect=fake_fetch),
        ),
        patch("worker.activities.synthesis_export.fetch._persist_sources", new=AsyncMock()),
    ):
...
async def test_fetch_sets_status_fetching():
    ...
    with patch(
        "worker.activities.synthesis_export.fetch.set_status", new=AsyncMock(),
    ) as flip, patch(
