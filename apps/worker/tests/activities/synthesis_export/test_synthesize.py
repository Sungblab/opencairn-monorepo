...
async def test_synthesize_returns_output_and_records_tokens():
    ...
    bundle = SourceBundle(
        items=[SourceItem(id="s", title="t", body="b", token_count=10, kind="note")]
    )
    ...
    with (
        patch(
            "worker.activities.synthesis_export.synthesize.set_status",
            new=AsyncMock(),
        ),
        patch(
        ...
        patch(
            "worker.activities.synthesis_export.synthesize.SynthesisExportAgent"
        ) as agent_cls,
        patch(
        ...
        agent.run = AsyncMock(
            return_value=(fake_output, UsageCounts(input_tokens=1234, output_tokens=560))
        )
...
@pytest.mark.asyncio
async def test_synthesize_sets_status_synthesizing():
    params = SynthesisRunParams(
        run_id="r1",
        workspace_id="w1",
        project_id=None,
    ...
    with (
        ...
        agent = agent_cls.return_value
        agent.run = AsyncMock(
            return_value=(fake_output, UsageCounts(input_tokens=1, output_tokens=1))
        )
        env = ActivityEnvironment()
        await env.run(synthesize_activity, params, bundle)
        flip.assert_awaited_once_with("r1", "synthesizing")
