...
async def test_enhance_activity_emits_enhancing_stage():
    ...
    with (
        ...
        patch("worker.activities.enhance_activity.publish_safe", side_effect=_publish_recorder(bag)),
...
async def test_create_source_note_emits_persisting_and_completed():
    ...
    with (
        patch(
            ...
            new=AsyncMock(return_value={"noteId": "11111111-1111-1111-1111-111111111111"}),
        ),
...
async def test_report_ingest_failure_emits_failed():
    ...
    with (
        patch("worker.activities.note_activity.post_internal", new=AsyncMock(return_value={})),
        patch("worker.activities.note_activity.publish_safe", side_effect=_publish_recorder(bag)),
    ):
