...
function parseArgs(args) {
  const options = { root: process.cwd(), limit: Number.POSITIVE_INFINITY, json: false };
...
function runCli() {
  ...
  if (rows.length === 0) {
    throw new Error(`No route client manifests found under ${path.join(options.root, ".next")}`);
  }
