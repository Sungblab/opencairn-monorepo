    ...
    try {
      ...
      if (!urlRes.ok) {
        ...
        if (body.error === "zip_too_large") {
          setError(t("errors.zipTooLarge"));
        } else {
          setError(t("progress.failed"));
        }
        return;
      }
      ...
      // XHR (not fetch) for upload-progress events. fetch doesn't expose a
      // progress hook in browsers yet.
    ...
    <div className="space-y-4">
      ...
      {file && uploading && (
        <div className="rounded border border-border p-4">
          <p className="text-sm">{t("notion.uploading", { progress })}</p>
