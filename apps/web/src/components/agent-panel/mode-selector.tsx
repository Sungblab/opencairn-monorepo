  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        sideOffset={6}
        className="w-36 rounded-[var(--radius-control)] border border-border bg-background p-1 shadow-sm ring-0"
      >
        {MODES.map((m) => (
          <DropdownMenuItem
            key={m}
            onSelect={() => onChange(m)}
            className={`flex min-h-8 items-center gap-2 rounded-[var(--radius-control)] px-2 py-1.5 text-xs uppercase transition-colors hover:bg-muted hover:text-foreground focus:bg-muted focus:text-foreground ${
