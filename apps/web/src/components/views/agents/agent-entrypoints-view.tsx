...
import type { WorkflowConsolePanelProps } from "./workflow-console-panel";
import { RunDetailSheetLoader } from "./run-detail-sheet-loader";
...
const WorkflowConsolePanel = dynamic<WorkflowConsolePanelProps>(
  () => import("./workflow-console-panel").then((mod) => mod.WorkflowConsolePanel),
  {
    ssr: false,
    loading: () => <WorkflowConsolePanelSkeleton />,
  },
);

function WorkflowConsolePanelSkeleton() {
  return (
    <section id="workflow-console" aria-hidden className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="h-8 w-72 animate-pulse rounded-[var(--radius-control)] bg-muted/80" />
      </div>
...
export interface AgentEntryPointsViewProps {
  projectId: string;
}

export function AgentEntryPointsView({ projectId }: AgentEntryPointsViewProps) {
    ...
    <div data-testid="route-agents" className="flex flex-col gap-6 p-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <WorkflowConsolePanel projectId={projectId} formatDate={formatDate} />
