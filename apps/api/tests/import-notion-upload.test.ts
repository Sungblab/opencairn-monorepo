...
describe("POST /api/import/notion/upload-url", () => {
  let seed: SeedResult;
  ...
  afterEach(async () => {
    await seed.cleanup();
    delete process.env.IMPORT_NOTION_ZIP_MAX_BYTES;
  });

  it("returns objectKey + presigned uploadUrl for a member with write access", async () => {
    const app = createApp();
    const res = await app.request("/api/import/notion/upload-url", {
      method: "POST",
      headers: {
    ...
    expect(json.objectKey).toMatch(
      new RegExp(
        `^imports/notion/${seed.workspaceId}/${seed.userId}/\\d+-[0-9a-f-]+\\.zip$`,
      ),
    );
    expect(json.uploadUrl).toMatch(/^https?:\/\//);
    // MinIO signs the object path into the URL — sanity check it targets our key.
    expect(json.uploadUrl).toContain(seed.workspaceId);
  });

  it("rejects sizes above the configured ceiling with 413", async () => {
    process.env.IMPORT_NOTION_ZIP_MAX_BYTES = String(1 * MB);
    const app = createApp();
    const res = await app.request("/api/import/notion/upload-url", {
      method: "POST",
      headers: {
  ...
  it("returns 400 when workspaceId is missing", async () => {
    ...
    const res = await app.request("/api/import/notion/upload-url", {
      ...
      body: JSON.stringify({ size: 10 * MB, originalName: "export.zip" }),
    });
    expect(res.status).toBe(400);
  });
});
