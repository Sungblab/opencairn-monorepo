...
- 🟡 Active / next: Literature Search & Auto-Import (12 task 완료, branch `feat/plan-literature-search` 워크트리, 미머지 — 0033 마이그), Live Ingest Visualization (백엔드 4 commits, branch `feat/live-ingest-visualization` 미머지), Synthesis Export (plan only, branch `feat/plan-synthesis-export` 미실행), Deep Research prod release (manual env flip).
