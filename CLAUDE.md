...
- 🟡 Active / next: Content-Aware Enrichment Spec B (8 task 완료, branch `feat/plan-content-enrichment` 워크트리, 미머지 — 0034 마이그, FEATURE_CONTENT_ENRICHMENT 기본 OFF), Live Ingest Visualization (백엔드 4 commits, branch `feat/live-ingest-visualization` 미머지), Synthesis Export (plan only, branch `feat/plan-synthesis-export` 미실행), Deep Research prod release (manual env flip).
