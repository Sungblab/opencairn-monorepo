...
- ✅ Complete: Plan 1, 13, 12, 3, 3b, 4, 9a, 2A (editor core, solo), 2B (Hocuspocus + comments + @mention), Ingest Source Expansion, Onboarding, Agent Runtime v2 Sub-A, React Email, Deep Research Phase A/B/C/D, App Shell Phase 1 (shell frame) + Phase 2 (sidebar) + Phase 3-A (tab bar chrome) + Phase 3-B (mode router + viewers), Plan 7 Phase 1 (Canvas web runtime + Tab Mode Router).
- 🟡 Active / next: Plan 2C (notifications + share), 2D (chat renderer + block extensions), App Shell Phase 4~5 (agent panel, palette), Plan 5/6/8, Plan 7 Phase 2 (Code Agent + /api/code/run + Monaco), Deep Research Phase E (BYOK UI · /settings/ai · prod release).
