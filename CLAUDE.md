...
- 🟡 Active / next: Plan 11B Phase A slash commands (plan only — DocEditorAgent + /improve·/translate·/summarize·/expand + InlineDiffSheet + doc_editor_calls, FEATURE_DOC_EDITOR_SLASH OFF), Synthesis Export (plan only, branch `feat/plan-synthesis-export` 미실행), Deep Research prod release (manual env flip).
